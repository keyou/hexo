...
const { extname, join, sep, dirname, basename } = require('path');
...
module.exports = ctx => {
  ...
  Post.virtual('path').get(function() {
    let path = ctx.execFilterSync('post_permalink', this, {context: ctx});
    path = path.split("/");
    path.splice(3,1);
    path = path.join("/");
  ...
  Post.virtual('asset_dir').get(function() {
    ...
    return src.substring(0, src.length - basename(src).length);
